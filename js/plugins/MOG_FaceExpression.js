...
/*:
 * @target MZ
 * @plugindesc (v1.0) Adiciona faces emotivas nos sprites dos characters.
 * @author Moghunter
 ...
 *
 * @command PlayerFace
 * @desc Ativar o efeito no jogador.
 * @text Player Emotion
 *
 * @arg id
 ...
 *
 * @arg motion
 * @desc Define o tipo de emoção.
 * (Normal) remove o efeito. 
 * @text Eyes
 * @default Normal (Cancel)
 * @type select
 * @option Normal (Cancel)
 * @value Normal (Cancel)
 * @option Look Right
 * @value Look Right
 * @option Look Left
 ...
 * 
 * @arg eleft
 * @desc Apresentar o efeito no olho direito.
 * @text Right Eye (Visible)
 * @default true
 * @type boolean 
 * 
 * @arg eright
 * @desc Apresentar o efeito no olho esquerdo.
 * @text Left Eye (Visible)
 * @default true
 * @type boolean  
 *
 * @arg x1
 * @desc Posição X do olho (Frontal).
 * @text X Offset (Frontal)
 * @default 0
 * @type number
 *
 * @arg y1
 * @desc Posição Y do olho (Frontal).
 * @text Y Offset (Frontal)
 * @default 0
 * @type number
  *
 * @arg x2
 * @desc Posição X do olho (Lateral).
 * @text X Offset (Lateral)
 * @default 0
 * @type number
 *
 * @arg y2
 * @desc Posição Y do olho (Lateral).
 * @text Y Offset (Lateral)
 * @default 0
 * @type number
 *
 * @command EventFace
 * @desc Ativar o efeito no evento.
 * @text Event Emotion
 *
 * @arg id
 ...
 * 
 * @arg eright
 * @desc Apresentar o efeito no olho esquedo.
 * @text Left Eye (Visible)
 * @default true
 * @type boolean  
 *
 ...
 * PLUGIN COMMANDS
 * =============================================================================
 * Utilize o Plugin Command para ativar os efeitos nos personagens e eventos.
 *
 * ============================================================================ 
 * TIPO DE EXPRESSÕES (EYES)
 * ============================================================================ 
 * 0  - Look Right
 * 1  - Look Left
 * 2  - Look Up
 ...
 * movimento.
 *
 * this.setFace(eyes,mouth,left_Eye,right_Eye)
 *
 * Ex
 * this.setFace(3,4,true,true) 
 *
 * ============================================================================
 ...
 * as faces.
 * 
 * face : EYES : MOUTH : L_EYE : R_EYE : X1_OFF : Y1_OFF : X2_OFF : Y2_OFF
 * this.setFace(eyes,mouth,left_Eye,right_Eye,x1_off,y1_off,x2_off,y2_off)
 *
 * EYES    - EYE_EXPRESSION
 ...
 *
 * Ex
 * face : 3 : 2 : true : true : 10 : 10 : 0 : 10
 * this.setFace(3,3,true,true,10,10,10,10)
 *
 * ============================================================================ 
 * * REMOVER AS EXPRESSÕES
 * ============================================================================ 
 * Para remover as expressões defina a face e a boca como NORMAL.
 * 
 */
...
　　Imported.MOG_FaceExpression = true;
　　var Moghunter = Moghunter || {}; 

  Moghunter.parameters = PluginManager.parameters('MOG_FaceExpression');  
  ...
  PluginManager.registerCommand('MOG_FaceExpression', "PlayerFace", data => {
		  var charID = Number(data.id);
		  var char = $gameMap.getPlayerChar(charID);
		  if (char) {$gameMap.setFaceDataChar(data,char)};	
   });
 
  PluginManager.registerCommand('MOG_FaceExpression', "EventFace", data => {
		  var charID = Number(data.id);
		  var char = $gameMap.getEventChar(charID);
		  if (char) {$gameMap.setFaceDataChar(data,char)};	
   });
...
// * getCharaterEmotion
//==============================
Game_Map.prototype.getCharaterEmotion = function(emotion) {
     if (emotion == "Look Right") {return 0;
	 } else if (emotion == "Look Left") {return 1;
	 } else if (emotion == "Look Up") {return 2;
	 } else if (emotion == "Look Down") {return 3;
	 } else if (emotion == "Scare") {return 4;	 
	 } else if (emotion == "Blink Left") {return 5;		 
	 } else if (emotion == "Blink Right") {return 6;		 
	 } else if (emotion == "Happy") {return 7;
	 } else if (emotion == "Angry") {return 8;
	 } else if (emotion == "Annoyed") {return 9;
	 } else if (emotion == "Crying") {return 10;
	 } else if (emotion == "Bitter") {return 11;
	 } else if (emotion == "Zombie") {return 12;
	 } else if (emotion == "Thinking") {return 13;
	 } else if (emotion == "Smug") {return 14;
	 } else if (emotion == "Satisfied") {return 15;
	 } else if (emotion == "Dead") {return 16;
	 };
	 return -1;
};
...
// * getCharaterMouth
//==============================
Game_Map.prototype.getCharaterMouth = function(mouth) {
     if (mouth == "Laugh") {return 0;
	 } else if (mouth == "Smile") {return 1;
	 } else if (mouth == "Sad") {return 2;
	 } else if (mouth == "Serious") {return 3;
	 } else if (mouth == "Open") {return 4; 
	 } else if (mouth == "Sorry") {return 5;
	 } else if (mouth == "Cat") {return 6;
	 };
	 return -1;
};
...
// * set Face Data Char
//==============================
Game_Map.prototype.setFaceDataChar = function(args,char) {
	   var mode = this.getCharaterEmotion(args.motion);
	   var mouth = this.getCharaterMouth(args.mouth);
	   if (mode < 0 && mouth < 0) {
		   if (!char._faceData.enabled) {return}; 
		  char._faceData.needRemove = true  
		  return;
	   };
	   char._faceData.enabled = true;
	   char._faceData.mode = Number(mode);
	   char._faceData.eyeL = String(args.eleft) == "true" ? true : false;
	   char._faceData.eyeR = String(args.eright) == "true" ? true : false;
	   char._faceData.mouth = Number(mouth);
	   char._faceData.x = Number(args.x1);
	   char._faceData.y = Number(args.y1);
	   char._faceData.x2 = Number(args.x2);
	   char._faceData.y2 = Number(args.y2);	
...
// * set Face
//==============================
Game_CharacterBase.prototype.setFace = function(mode,mouth,eleft,eright,x1,y1,x2,y2) {
	   var mode = mode != null ? mode : 0;
	   var mouth = mouth != null ? mouth : 0;
	   ...
	   this._faceData.mouth = Number(mouth);
	...
	if (!this._erased && this.page()) {this.list().forEach(function(l) {
	       if (l.code === 108) {var comment = l.parameters[0].split(' : ')
			    if (comment[0].toLowerCase() == "face"){
					...
					var mouth = comment[2] != null ? comment[2] : -1;				
				    var eleft = comment[3] != null ? comment[3] : true;
				    var eright = comment[4] != null ? comment[4] : true;	   
				    var x1 = comment[5] != null ? comment[5] : 0;
				    var y1 = comment[6] != null ? comment[6] : 0;
				    var x2 = comment[7] != null ? comment[7] : 0;
				    var y2 = comment[8] != null ? comment[8] : 0;					
    				this._faceData.enabled = true;
					this._faceData.mode = Number(mode);	
					this._faceData.mouth = Number(mouth);	
				    this._faceData.eyeL = String(eleft) == "true" ? true : false;
...
// * mouth Mode
//==============================
Sprite_Character.prototype.mouthMode = function() {
   return this._character._faceData.mouth;
};
//==============================
// * set Face Pos X
//==============================
...
Sprite_Character.prototype.setFaceFrame = function(sprite) {
	...
	sprite.visible = true;
	if (sprite.index == 4) {
		this.setBitmapMouth(sprite,sx,sy,pw,ph)	   
	} else if (sprite.index < 2 && this.faceMode() >= 0) {
		this.setBitmapEyesBack(sprite,sx,sy,pw,ph)
	} else {
	if (this.faceMode() >= 0) {this.setBitmapEyes(sprite,sx,sy,pw,ph)}	
	};	
    if (sprite.index == 4) {
        this.setMouthPosition(sprite);
    } else {
...
// * setBitmapMouth
//==============================
Sprite_Character.prototype.setBitmapMouth = function(sprite,sx,sy,pw,ph) {	
	if (this._character.direction() == 2) {
	    this.setBitmapMouthFront(sprite,sx,sy,pw,ph);
	} else {
		this.setBitmapMouthSide(sprite,sx,sy,pw,ph);
	};
};
...
Sprite_Character.prototype.setBitmapEyesBackFront = function(sprite,sx,sy,pw,ph) {	
		...
		if (this.faceMode() == 5) {
			...
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,6);
			var color = "#5C3A1C"
			if (sprite.index == 1) {
				sprite.bitmap.fillRect(3,0,2,1,color);	
				sprite.bitmap.fillRect(1,1,3,1,color);
				sprite.bitmap.fillRect(0,2,2,1,color);
				sprite.bitmap.fillRect(1,3,3,1,color);
				sprite.bitmap.fillRect(3,4,2,1,color);
			} else {
     			sprite.bitmap.fillRect(1,0,2,1,color);
	    		sprite.bitmap.fillRect(2,1,3,1,color);
		    	sprite.bitmap.fillRect(4,2,2,1,color);
			    sprite.bitmap.fillRect(2,3,3,1,color);
			    sprite.bitmap.fillRect(1,4,2,1,color);				
			};			
		} else if (this.faceMode() == 13) {	
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,6);
			var color = "#5C3A1C"
			sprite.bitmap.fillRect(0,3,6,1,color);
		} else if (this.faceMode() == 14) {	
			var color = "#000000"
			sprite.bitmap.fillRect(0,1,6,1,color);				
		} else if (this.faceMode() == 15) {	
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,6);
			var color = "#5C3A1C"
			if (sprite.index == 1) {
               sprite.bitmap.fillRect(0,2,5,2,color);
			} else {
		       sprite.bitmap.fillRect(1,2,5,2,color);				
			};
		} else if (this.faceMode() == 16) {	
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,6);
			var color = "#FFFFFF"
			sprite.bitmap.fillRect(0,0,5,5,color);
			var color = "#5C3A1C"
	        sprite.bitmap.fillRect(4,0,2,1,color);	
			sprite.bitmap.fillRect(3,1,2,1,color);
			sprite.bitmap.fillRect(2,2,2,1,color);
			sprite.bitmap.fillRect(1,3,2,1,color);
			sprite.bitmap.fillRect(0,4,2,1,color);
			sprite.bitmap.fillRect(4,4,2,1,color);
			sprite.bitmap.fillRect(3,3,2,1,color);
			sprite.bitmap.fillRect(2,2,2,1,color);
			sprite.bitmap.fillRect(1,1,2,1,color);
			sprite.bitmap.fillRect(0,0,2,1,color);				
		} else {
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,4);
            this.setBitmapEyeBackWhite(sprite,5,4);
		};
...
// * set Bitmap Eyes Back Front
//==============================	
Sprite_Character.prototype.setBitmapMouthFront = function(sprite,sx,sy,pw,ph) {	
		if (this.patternHeight() >= 60) {
			var bx = pw + Moghunter.face_RectX_64_F1;
			var by = ph - this.patternHeight() + Moghunter.face_RectY_64_F1;
			var wx = 1;
	    ...
	    var color = "#500000"		
		if (this.mouthMode() == 0) {	
	        sprite.bitmap.fillRect(5,6,7,1,color);
		    sprite.bitmap.fillRect(6,7,5,1,color);		
		} else if (this.mouthMode() == 1) {	
	    	sprite.bitmap.fillRect(5,7,1,1,color);	
			sprite.bitmap.fillRect(6,8,5,1,color);
		 	sprite.bitmap.fillRect(11,7,1,1,color);
		} else if (this.mouthMode() == 2) {	
	    	sprite.bitmap.fillRect(5,8,1,1,color);	
			sprite.bitmap.fillRect(6,7,5,1,color);
		 	sprite.bitmap.fillRect(11,8,1,1,color);
		} else if (this.mouthMode() == 3) {	
			sprite.bitmap.fillRect(6,7,5,1,color);
		} else if (this.mouthMode() == 4) {	
			sprite.bitmap.fillRect(6,7,5,2,color);			
		} else if (this.mouthMode() == 5) {	
			sprite.bitmap.fillRect(7,7,2,2,color);
		} else if (this.mouthMode() == 6) {	
			sprite.bitmap.fillRect(5,6,1,2,color);
	     	sprite.bitmap.fillRect(6,8,2,1,color);
			sprite.bitmap.fillRect(8,7,1,1,color);
			sprite.bitmap.fillRect(9,8,2,1,color);
			sprite.bitmap.fillRect(11,6,1,2,color);
		} else {
...
// * setBitmapMouthSide
//==============================	
Sprite_Character.prototype.setBitmapMouthSide = function(sprite,sx,sy,pw,ph) {	
		if (this.patternHeight() >= 60) {
			var bx = pw + Moghunter.face_RectX_64_F1;
			var by = ph - this.patternHeight() + Moghunter.face_RectY_64_F1;
			var wx = 1;
    	...
    	var color = "#500000"		
		if (this.mouthMode() == 0) {
	        sprite.bitmap.fillRect(5,6,6,1,color);
	    	sprite.bitmap.fillRect(6,7,4,1,color);	
		} else if (this.mouthMode() == 1) {	
           if (this._character.direction() == 4) {	
			   sprite.bitmap.fillRect(8,8,4,1,color);
		 	   sprite.bitmap.fillRect(12,7,1,1,color);	
		   } else {
			   sprite.bitmap.fillRect(3,8,4,1,color);
		 	   sprite.bitmap.fillRect(2,7,1,1,color);			   
		   }
		} else if (this.mouthMode() == 2) {	
           if (this._character.direction() == 4) {	
			   sprite.bitmap.fillRect(8,7,4,1,color);
		 	   sprite.bitmap.fillRect(12,8,1,1,color);	
		   } else {
			   sprite.bitmap.fillRect(3,7,4,1,color);
		 	   sprite.bitmap.fillRect(2,8,1,1,color);			   
		   };		   
		} else if (this.mouthMode() == 3) {	
           if (this._character.direction() == 4) {	
			   sprite.bitmap.fillRect(8,7,5,1,color);
		   } else {
			   sprite.bitmap.fillRect(3,7,5,1,color);
		   };
		} else if (this.mouthMode() == 4) {	
           if (this._character.direction() == 4) {	
			   sprite.bitmap.fillRect(8,7,5,2,color);
		   } else {
			   sprite.bitmap.fillRect(3,7,5,2,color);
		   };
		} else if (this.mouthMode() == 5) {	
           if (this._character.direction() == 4) {	
			   sprite.bitmap.fillRect(8,7,2,2,color);
		   } else {
			   sprite.bitmap.fillRect(5,7,2,2,color);
		   };
		} else if (this.mouthMode() == 6) {	
           if (this._character.direction() == 4) {	
			sprite.bitmap.fillRect(6,6,1,2,color);
	     	sprite.bitmap.fillRect(7,8,1,1,color);
			sprite.bitmap.fillRect(8,7,1,1,color);
			sprite.bitmap.fillRect(9,8,2,1,color);
			sprite.bitmap.fillRect(11,6,1,2,color);	
		   } else {
			sprite.bitmap.fillRect(4,6,1,1,color);
	     	sprite.bitmap.fillRect(5,8,2,1,color);
			sprite.bitmap.fillRect(7,7,1,1,color);
			sprite.bitmap.fillRect(8,8,1,1,color);
			sprite.bitmap.fillRect(9,6,1,2,color);		
		   };			   
		} else {
		  sprite.visible = false;
		};				
...
Sprite_Character.prototype.setBitmapEyesBackSide = function(sprite,sx,sy,pw,ph) {	
		...
		if (this.patternHeight() >= 60) {
     	...
     	if (this.faceMode() == 14) {
           sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,3,6,3);
		} else if (this.faceMode() == 11) {
			sprite.bitmap.blt(this.bitmap,sx + bx ,sy + by,1,1,0,0,7,6);		   
		} else if (this.faceMode() == 13) {
			sprite.bitmap.blt(this.bitmap,sx + bx ,sy + by,1,1,0,0,7,6);			   
		} else if (this.faceMode() == 15) {
			sprite.bitmap.blt(this.bitmap,sx + bx ,sy + by,1,1,0,0,7,6);
		...
		if (this.faceMode() == 5) {
			...
			if (sprite.index == 1) {
				...
				sprite.bitmap.fillRect(2,2,5,1,color);
			};
		} else if (this.faceMode() == 14) {	
	    	var color = "#B08C5A"
			sprite.bitmap.fillRect(0,4,6,1,color);		
		} else if (this.faceMode() == 15) {	
	    	var color = "#5C3A1C"
			 if (sprite.index == 1) {
            	 sprite.bitmap.fillRect(0,2,4,2,color);
	    		 sprite.bitmap.fillRect(4,3,2,2,color);
			 } else {
            	 sprite.bitmap.fillRect(3,2,4,2,color);
	    		 sprite.bitmap.fillRect(1,3,2,2,color);
			 };
		} else if (this.faceMode() == 16) {	
			sprite.bitmap.blt(this.bitmap,sx + bx,sy + by,1,1,0,0,6,6);
			var color = "#FFFFFF"
		...
		sprite.y = ry + yf + this._character._faceData.y2;	
		 if (this.faceMode() == 6 || this.faceMode() == 8) {
			sprite.visible = false;	
     		 ...
     		 sprite.x = xf + rx + this._character._faceData.x2 - 2;	
			 sprite.y = yf + ry + this._character._faceData.y2 - 1;	
		 } else if (this.faceMode() == 13) {
     		 sprite.x = xf + rx + this._character._faceData.x2 - 2;	
			 sprite.y = yf + ry + this._character._faceData.y2 - 1;				 
		 } else if (this.faceMode() == 14) {
		...
		sprite.y = ry + yf + this._character._faceData.y2;
		 if (this.faceMode() == 5 || this.faceMode() == 8) {
			sprite.visible = false;	 
     		 ...
     		 sprite.x = xf + rx + this._character._faceData.x2 - 2;	
			 sprite.y = yf + ry + this._character._faceData.y2;	
		 } else if (this.faceMode() == 13) {
     		 sprite.x = xf + rx + this._character._faceData.x2 - 2;	
			 sprite.y = yf + ry + this._character._faceData.y2;			 
		 } else if (this.faceMode() == 14) {
			 ...
			 sprite.y = yf + ry + this._character._faceData.y2;
     		 ...
     		 sprite.x = xf + rx + this._character._faceData.x2 - 1;	
			 sprite.y = yf + ry + this._character._faceData.y2 - 1;				 			 			 		
    	 };
	} else {
	    if (this.faceMode() == 0) {
...
Sprite_Character.prototype.setFacePosFront = function(sprite) {
	...
	if (sprite.index < 2) {
	    ...
	    if (this.faceMode() == 5 && sprite.index == 0) {	
    		...
    		sprite.y = ry + yf + this._character._faceData.y - 2;			
		} else if (this.faceMode() == 12) {
    		sprite.y = ry + yf + this._character._faceData.y - 2;		
		} else if (this.faceMode() == 13) {
    		sprite.y = ry + yf + this._character._faceData.y - 2;						
		} else if (this.faceMode() == 14) {
    		sprite.y = ry + yf + this._character._faceData.y + 1;			
		} else if (this.faceMode() == 15) {
    		sprite.y = ry + yf + this._character._faceData.y - 2;
		} else if (this.faceMode() == 16) {
    		sprite.y = ry + yf + this._character._faceData.y - 2;
		...
		if (this.faceMode() == 0) {
		   ...
		   sprite.x = this.setFacePosX(sprite,xf,rx,1);
		   sprite.y = this.setFacePosY(sprite,yf + 5 ,ry + 5 ,0) -14;	
		} else if (this.faceMode() == 13) {
		   sprite.x = this.setFacePosX(sprite,xf,rx,1);
		   sprite.y = this.setFacePosY(sprite,yf + 5 ,ry + 5 ,0) -14;		   
		} else if (this.faceMode() == 15) {
...
// * setMouthPosition
//==============================
Sprite_Character.prototype.setMouthPosition = function(sprite) {
	sprite.visible = true;
	if (this._character.direction() == 8) {
		sprite.visible = false;
	} else if (this._character.direction() == 4) {
	    this.setMouthPosLeft(sprite); 
	} else if (this._character.direction() == 6) {
	    this.setMouthPosRight(sprite);
	} else  {
	     this.setMouthPosFront(sprite);
	};		
};
...
// * setMouthPosFront
//==============================
Sprite_Character.prototype.setMouthPosFront = function(sprite) {
	var bsize = this.patternHeight() < 60 ? false : true;
	var esX = !bsize ? Moghunter.face_PosX_48_F : Moghunter.face_PosX_64_F;
	var esY = !bsize ? Moghunter.face_PosY_48_F : Moghunter.face_PosY_64_F;
	var xf = (this.patternWidth() / 2) + esX;
	...
	var rx = !bsize ? -9 : -8;
	var ry = !bsize ? -26 : -38;
	sprite.x = rx + xf + this._character._faceData.x;	
	sprite.y = -1 + ry + yf+ this._character._faceData.y;	
};
...
// * setMouthPosLeft
//==============================
Sprite_Character.prototype.setMouthPosLeft = function(sprite) {
	var bsize = this.patternHeight() < 60 ? false : true
	var esX = !bsize ? Moghunter.face_PosX_48_S : Moghunter.face_PosX_64_S;
	var esY = !bsize ? Moghunter.face_PosY_48_S : Moghunter.face_PosY_64_S;	
	var xf = (this.patternWidth() / 2) - esX;
	...
	var rx = !bsize ? -8 : -7;
	var ry = !bsize ? -26 : -38;
	sprite.x = -7 + rx + xf - this._character._faceData.x2;	
	sprite.y = ry + yf + this._character._faceData.y2;	
};
...
// * setMouthPosRight
//==============================
Sprite_Character.prototype.setMouthPosRight = function(sprite) {
	var bsize = this.patternHeight() < 60 ? false : true
	var esX = !bsize ? Moghunter.face_PosX_48_S : Moghunter.face_PosX_64_S;
	var esY = !bsize ? Moghunter.face_PosY_48_S : Moghunter.face_PosY_64_S;	
	var xf = (this.patternWidth() / 2) - esX;
	...
	var rx = !bsize ? -8 : -7;
	var ry = !bsize ? -26 : -38;
	sprite.x = 6 + rx + xf + this._character._faceData.x2;
	sprite.y = ry + yf + this._character._faceData.y2;	
};
